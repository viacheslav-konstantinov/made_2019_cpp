...
class BigInteger
{
    public:
        ...
        // перегрузка для int&& не требуется
        // перемещение = копирование для встроенных типов, нет смысла в перегрузке
        // для аргумента в BigInteger(2) получается константная ссылка
        // и исользуется BigInteger(const int& other)
