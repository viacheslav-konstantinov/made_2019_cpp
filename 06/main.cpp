...
class BigInteger
{
    public:
        ...
        BigInteger()
            : digitLength_(0), isNegative_(false), digits_(nullptr) 
            {
        ...
        explicit BigInteger(BigInteger&& inputInteger);
        explicit BigInteger(const BigInteger& other);
        BigInteger& operator=(const BigInteger& other);
        BigInteger& operator=(BigInteger&& other);

        explicit BigInteger(int&& inputInteger);
        explicit BigInteger(const int& other);
...
{
    //начальная инициализация текщего объекта
    isNegative_ = false;
    digitLength_ = 0;
    digits_ = nullptr;
    ...
    // выставление флага отрицательности
    if (buffer < 0) 
    {
        isNegative_ = true;
        buffer = -buffer;
...
{
    // аналогично BigInteger::BigInteger(int&& other)
    isNegative_ = false;
    digitLength_ = 0;
    digits_ = nullptr;
...
BigInteger& BigInteger::operator=(BigInteger&& other)
{
    ...
    // удаление разрадов из текущего объекта
    delete[] digits_;
...
{
    // аналогично BigInteger::BigInteger(int&& other)
    // без начальной иниализации текущего объекта
...
{
    //аналогично BigInteger& BigInteger::operator=(BigInteger&& other)
    //без зануления other
    if(*this == other)
        return *this;
...
{
    //аналогично BigInteger& BigInteger::operator=(int&& other)
    //без зануления other
    int buffer(other);
    size_t necessaryDigits;
    if (buffer != 0)
